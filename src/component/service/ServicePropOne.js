				...
				<div className={`${colSize} ${topMargin(index)}`} key={index} >
					<div className={`services-grid ${serviceStyle}`}>
						...
						<div className="content">
							<h5 className="title"> 
								<Link to={process.env.PUBLIC_URL + `/tjeneste-detaljer/${slugify(data.title)}`}>{data.title}</Link>
							</h5>
							<p>{data.description}</p>
							<Link to={process.env.PUBLIC_URL + `/tjeneste-detaljer/${slugify(data.title)}`} className="more-btn">Finn ut mer</Link>
						</div>
					</div>
			 	</div>
			))}
